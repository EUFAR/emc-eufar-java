...
public class Emc_eufar implements EntryPoint {
	...
	private HashMap<DateBox, Label> correctDate = new HashMap<DateBox, Label>();
	private String emcVersion = new String("v0.9.4");
	...
	private String titleString = new String("EUFAR Metadata Creator");
	...
	public void onModuleLoad() {
		...
		correctDate.put(refPublicationDat,refPublicationLab);
		correctDate.put(refRevisionDat,refRevisionLab);
		correctDate.put(refCreationDat,refCreationLab);
		correctDate.put(refStartDat,refPhaseLab);
		correctDate.put(refEndDat,refPhaseLab);


		// Commands in the menu bar
		Command aboutWindow = new Command() {public void execute() {aboutWindow();}};
		Command aboutStandard = new Command() {public void execute() {Window.open("http://inspire.ec.europa.eu/", "_blank", "");}};
		Command newFile = new Command() {public void execute() {newFile();}};
		Command openFile = new Command() {public void execute() {openFile();}};
		Command saveFile = new Command() {public void execute() {runCheck("");}};
		...
		airAircraftLst.addChangeHandler(new ChangeHandler() {
			@Override
			public void onChange(ChangeEvent event) {airInformationSet(airAircraftLst.getSelectedIndex());}
		});
		...
		geoResolutionLst.addChangeHandler(new ChangeHandler() {
			@Override
			public void onChange(ChangeEvent event) {geoResolutionSet(geoResolutionLst.getSelectedIndex());}
		});
		...
		dockPanel.addEast(new HTML("<img src='icons/emc_shadowr.png' alt='ASMM Creator' width='30px' height='" + screen_height + "' align='left'"
				+ ">Development version of EMC, online version " + emcVersion), expandPanel);
		dockPanel.addWest(new HTML("<img src='icons/emc_shadowl.png' alt='ASMM Creator' width='30px' height='" + screen_height + "' align='right'"
				+ ">Development version of EMC, online version " + emcVersion), expandPanel);
	...
	private PushButton addInfoButton(final String context) {
		...
		infoButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				horizontalPanel01.add(addInfoText(context));
	...
	private Label addInfoText(final String context) {
		...
		case IDTITLE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is a characteristic, and often a unique name identifying the resource. "
					+ "The value domain of this metadata element is free text.</p><u>EUFAR:</u><br>Project acronym - survey request form.");
			break;
		case IDABSTRACT:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is a brief narrative summary of the content of the resource. The value "
					+ "domain of this metadata element is free text.</p><u>EUFAR:</u><br>Abstract - survey request form.");
			break;
		case IDTYPE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the type of resource being described by the metadata. The value "
					+ "domain of this metadata element is defined in Part D.1.</p><u>EUFAR:</u><br>Dataset or series.");
			break;
		case IDLOCATOR:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The resource locator defines the link(s) to the resource and/or the link to "
					+ "additional information about the resource. The value domain of this metadata element is a character string, commonly "
					+ "expressed as uniform resource locator (URL).</p><u>EUFAR:</u><br>Link to data in EUFAR N8-DB or http://www.eufar.net.");
			break;
		case IDIDENTIFIER:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>A value uniquely identifying the resource. The value domain of this metadata "
					+ "element is a mandatory character string code, generally assigned by the data owner, and a character string namespace "
					+ "uniquely identifying the context of the identifier code (for example, the data owner).</p><u>EUFAR:</u><br>N8-DB file "
					+ "name specification.");
			break;
		case IDLANGUAGE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The language(s) used within the resource. The value domain of this metadata "
					+ "element is limited to the languages defined in ISO 639-2.</p><u>EUFAR:</u><br>English.");
			break;
		case CATEGORIES:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The topic category is a high-level classification scheme to facilitate the "
					+ "grouping and topic-based search of available spatial data resources. The value domain of this metadata element is defined "
					+ "in Part D.2.</p><u>EUFAR:</u><br>Main scientific field -- survey request form.");
			break;
		case KEYWORDS:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The keyword value is a commonly used word, formalised word or phrase used to "
					+ "describe the subject. While the topic category is too obscure for detailed queries, keywords can help narrowing a full "
					+ "text search and allow for structured keyword searches. The value domain of this metadata element is free text.</p><p "
					+ "align=justify><u>EUFAR:</u><br>Keywords are from NASA/Global Change Master Directory (GCMD) Earth Science Keywords.</p>");
			break;
		case AIAIRCRAFT:
			infoLabel.setHTML("<p align=justify><u>Specific to EUFAR:</u><br>This is the aircraft used to acquire the actual data and metadata."
					+ "</p>");
			break;
		case AIINSTRUMENT:
			infoLabel.setHTML("<p align=justify><u>Specific to EUFAR:</u><br>This(these) is(are) the instrument(s) used to acquire the actual "
					+ "data and metadata.</p>");
			break;
		case GILOCATION:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the name of the geographic space covered by the dataset.</p><p "
					+ "align=justify><u>EUFAR:</u><br>Name of the geographic space -- survey request form.</p>");
			break;
		case GIBOX:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the extent of the resource in the geographic space, given as a "
					+ "bounding box. The bounding box shall be expressed with westbound and eastbound longitudes (-180°W to 180°E), and "
					+ "southbound and northbound latitudes (90°N to -90°S) in decimal degrees, with a precision of at least two decimals.</p><p "
					+ "align=justify><u>EUFAR:</u><br>Result of data processing plus information on country and region (survey request form)."
					+ "</p>");
			break;
		case GIUNIT:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>Spatial resolution refers to the level of detail of the data set. It shall be "
					+ "expressed as a set of zero up to many resolution distances (typically for gridded data and imagery-derived products) or "
					+ "equivalent scales (typically for maps or map-derived products). An equivalent scale is generally expressed as an integer "
					+ "value expressing the scale denominator. A resolution distance shall be expressed as a numerical value associated with a "
					+ "unit of length.</p><p align=justify><u>EUFAR:</u><br>Result of data processing.</p>");
			break;
		case TRPUBLICATION:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the date of publication of the resource when available, or the date "
					+ "of entry into force. There may be more than one date of publication.</p><p align=justify><u>EUFAR:</u><br>Date of entry "
					+ "into EUFAR N8-DB. The date have to be set 'before' the current date.</p>");
			break;
		case TRREVISION:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the date of last revision of the resource, in the case that the "
					+ "resource has been revised. There shall not be more than one date of last revision.</p><p align=justify><u>EUFAR:</u><br>"
					+ "Automatically created within EUFAR N8-DB (date stamp). The date have to be set 'before' the current date.</p>");
			break;
		case TRCREATION:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the date of creation of the resource. There shall not be more than "
					+ "one date of creation.</p><p align=justify><u>EUFAR:</u><br>Date of data processing (related to product, which is stored "
					+ "in EUFAR N8-DB). The date have to be set 'before' the current date.</p>");
			break;
		case TRPHASE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The temporal extent defines the time period covered by the content of the "
					+ "resource. This time period may be expressed as any of the following: an individual date, an interval of dates expressed "
					+ "represented by the starting date and end date of the interval, or a mix of individual dates and intervals of dates.</p><p "
					+ "align=justify><u>EUFAR:</u><br>Date of data acquisition.</p>");
			break;
		case AUCONDITIONS:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This metadata element defines the conditions for access and use of spatial "
					+ "data sets and services, and where applicable, corresponding fees as required by Article 5(2)(b) and Article 11(2)(f) of "
					+ "Directive 2007/2/EC. The value domain of this metadata element is free text. The element must have values. If no "
					+ "conditions apply to the access and use of the resource, 'no conditions apply' shall be used. If conditions are unknown, "
					+ "'conditions unknown' shall be used. This element shall also provide information on any fees necessary to access and use "
					+ "the resource, if applicable, or refer to a uniform resource locator (URL) where information on fees is available.</p><p "
					+ "align=justify><u>EUFAR:</u><br>Consortium agreement for access usage restrictions.</p>");
			break;
		case AULIMITATIONS:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>When Member States limit public access to spatial data sets and spatial data "
					+ "services under Article 13 of Directive 2007/2/EC, the metadata element shall provide information on the limitations and "
					+ "the reasons for this. If there are no limitations on public access, the metadata element shall indicate this. The value "
					+ "domain of the metadata element is free text.</p><p align=justify><u>EUFAR:</u><br>Consortium agreement for public access "
					+ "limitations.</p>");
			break;
		case ROPARTY:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the description of the organisation responsible for the establishment, "
					+ "management, maintenance and distribution of the resource. This description shall include:<ul><li>the name of the "
					+ "organisation as free text</li><li>a contact e-mail address as a character string</li></ul><p align=justify><u>EUFAR:</u>"
					+ "<br>Party, who created the resource/data provider.</p>");
			break;
		case ROEMAIL:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the description of the organisation responsible for the establishment,"
					+ " management, maintenance and distribution of the resource. This description shall include:<ul><li>the name of the "
			...
			break;
		case ROROLE:
			infoLabel.setHTML("'<p align=justify><u>INSPIRE:</u><br>This is the role of the responsible organisation. The value domain of this "
					+ "metadata element is defined in Part D.</p><p align=justify><u>EUFAR:</u><br>pointOfContact and principalInvestigator (two "
					+ "different entries).</p>");
			break;
		case MMDATE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>The date which specifies when the metadata record was created or updated. "
					+ "This date shall be expressed in conformity with ISO 8601.</p><p align=justify><u>EUFAR:</u><br>Date when the metadata "
					+ "record was created or updated (result of data processing or entry into DB).</p>");
			break;
		case MMLANGUAGE:
			infoLabel.setHTML("<p align=justify><u>INSPIRE:</u><br>This is the language in which the metadata elements are expressed. The value "
					+ "domain of this metadata element is limited to the official languages of the Community and is expressed in conformity with "
					+ "ISO 639-2.</p><p align=justify><u>EUFAR:</u><br>English.</p>");
			break;
		case MMPARTY:
			infoLabel.setHTML("<p align=justify><u>INSPIRE & EUFAR:</u><br>This is the name and the contact e-mail of the party responsible for "
					+ "the data set.</p>");
			break;
		default:
	...
	private PushButton addMetPlusButton() {
		...
		plusButton.addClickHandler(new ClickHandler() {
			public void onClick(ClickEvent event) {
				...
				metNameLst.add(metNameBox);
				metEmailLst.add(metEmailBox);
				verticalPanel01.add(metPartyTab);
				metNameLab.setStyleName("metTextLabel");
				...
				metAddTab.setWidget(row, 0, verticalPanel01);
	...
	private void aboutWindow() {
		...
		final HTML label = new HTML("<p align=justify>This is the EUFAR Metadata Creator " + emcVersion + ". It was developed by EUFAR using "
				+ "Java and Google Web Toolkit. The goal of the EUFAR Metadata Creator is to produce metadata to facilitate data storage and "
				+ "searches for Airborne Scientific Campaigns. XML files generated by this version conform to " + inspireVersion + " of the "
				+ "INSPIRE metadata and XML Standard.</p><p>For any questions, more information, or to submit a bug report, please contact <a "
				+ "href=\"mailto:xxxxxxxxxxxx@xxxxxxx\"><span style=\" text-decoration: underline; color:#0000ff;\">xxxxxxxxxxxxxxxxxxxxx</span>"
				+ "</a>.</p><p>The latest version and source code of the EUFAR Metadata Creator can be found at <a href=https://github.com/eufar"
		...
		button.getElement().setAttribute("style", "margin-left: 170px !important; font-family: DroidSansFallback !important; font-weight: bold "
	...
	private void clearFields() {
		...
		for (int i = 0; i < allLabel.size(); i++) {allLabel.get(i).getElement().setAttribute("style","color: black !important;");}
		...
		for (int i = 0; i < allDateBox.size(); i = i + 1) {allDateBox.get(i).setValue(new Date());}
	...
	private void saveFile(final String string) {
		...
		submitButton.getElement().setAttribute("style", "margin-left:19px !important; font-family: DroidSansFallback !important; font-weight: "
				+ "bold !important;");
	...
	private String createXml() {
		...
		add_element(doc, "gco:Date", identPubDate, DateTimeFormat.getFormat("yyyy-MM-dd").format(refPublicationDat.getValue()));
		...
		add_element(doc, "gco:Date", identRevDate, DateTimeFormat.getFormat("yyyy-MM-dd").format(refRevisionDat.getValue()));
		...
		add_element(doc, "gco:Date", identCreaDate, DateTimeFormat.getFormat("yyyy-MM-dd").format(refCreationDat.getValue()));
		...
		add_element(doc, "gco:CharacterString", identThesaurusTitle, "NASA/Global Change Master Directory (GCMD) Earth Science Keywords. Version "
				+ "8.0.0.0.0");
		...
		Element identThesaurusDateTypeCI = add_element(doc, "gmd:CI_DateTypeCode", identThesaurusDateType, "revision");
		...
		for (int i = 0; i < refEndLst.size(); i++) {
			...
			add_element(doc, "gml:beginPosition", identExtTempPeriod, DateTimeFormat.getFormat("yyyy-MM-dd").format(refStartLst.get(i).getValue()));
			add_element(doc, "gml:endPosition", identExtTempPeriod, DateTimeFormat.getFormat("yyyy-MM-dd").format(refEndLst.get(i).getValue()));
		}
		...
		for (int i = 0; i < useLimitationsLst.size(); i++) {
			if (i > 0) {
				if (useLimitationsLst.get(i).getText() != "") {
					...
					add_element(doc, "gco:CharacterString", identLimitationsUse, useLimitationsLst.get(i).getText());
				}
			} else {
				Element identLimitationsUse = add_element(doc, "gmd:otherConstraints", identLegalConstraintMD);
				add_element(doc, "gco:CharacterString", identLimitationsUse, useLimitationsLst.get(i).getText());
		...
		add_element(doc, "gco:CharacterString", titleQuality, "COMMISSION REGULATION (EC) No 1205/2008 of 3 December 2008 implementing Directive "
		...
		try {
			...
			for (int aa = 0; aa < docChildNodes.getLength(); aa++) {
					...
					for (int bb = 0; bb < dataIdentification.getLength(); bb++) {
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:citation")) {
							...
							for (int cc = 0; cc < citation.getLength(); cc++) {
								if (citation.item(cc).getNodeName().startsWith("gmd:title")) {
									try {identTitleBox.setText(citation.item(cc).getFirstChild().getFirstChild().getNodeValue());}
									catch (Exception ex) {}
								}
								if (citation.item(cc).getNodeName().startsWith("gmd:identifier")) {
									try {
										identIdentifierBox.setText(citation.item(cc).getFirstChild().getFirstChild().getFirstChild().
								...
								if (citation.item(cc).getNodeName().startsWith("gmd:date")) {
									...
									if (dateTypeCode == "revision") {
										refRevisionDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(dateValue));
									}
									else if (dateTypeCode == "creation") {
										refCreationDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(dateValue));
									}
									else if (dateTypeCode =="publication") {
										refPublicationDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(dateValue));
									}
								}
							}
						}
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:abstract")) {
							try {identAbstractAre.setText(dataIdentification.item(bb).getFirstChild().getFirstChild().getNodeValue());}
							catch (Exception ex) {}
						}
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:language")) {
							checkList(languageMap, dataIdentification.item(bb).getFirstChild().getFirstChild().getNodeValue(), identLanguageLst);
						}
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:topiccategory")) {
							checkBox(clScroll, categoriesMap, dataIdentification.item(bb).getFirstChild().getFirstChild().getNodeValue());
						}
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:descriptivekeywords")) {
							...
							for (int dd = 0; dd < keywords.getLength(); dd++) {
								if (keywords.item(dd).getNodeName().startsWith("gmd:keyword")) {
									checkBox(kwScroll, keywordsMap, keywords.item(dd).getFirstChild().getFirstChild().getNodeValue());
								}
							}
						}
							...
							for (int ee = 0; ee < extents.getLength(); ee++) {
								...
								if (extents.item(ee).getNodeName().startsWith("gmd:geographicelement")) {
									...
									for (int ff = 0; ff < geoExtents.getLength(); ff++) {
										if (geoExtents.item(ff).getNodeName().startsWith("gmd:westboundlongitude")) {
											try {geoWestBox.setText(geoExtents.item(ff).getFirstChild().getFirstChild().getNodeValue());}
											catch (Exception ex) {}
										}
										if (geoExtents.item(ff).getNodeName().startsWith("gmd:eastboundlongitude")) {
											try {geoEastBox.setText(geoExtents.item(ff).getFirstChild().getFirstChild().getNodeValue());}
											catch (Exception ex) {}
										}
										if (geoExtents.item(ff).getNodeName().startsWith("gmd:northboundlatitude")) {
											try {geoNorthBox.setText(geoExtents.item(ff).getFirstChild().getFirstChild().getNodeValue());}
											catch (Exception ex) {}
										}
										if (geoExtents.item(ff).getNodeName().startsWith("gmd:southboundlatitude")) {
											try {geoSouthBox.setText(geoExtents.item(ff).getFirstChild().getFirstChild().getNodeValue());}
											catch (Exception ex) {}
										}
									}
								}
								if (extents.item(ee).getNodeName().startsWith("gmd:temporalelement")) {
									...
									String periodEnd = extents.item(ee).getFirstChild().getFirstChild().getFirstChild().getLastChild().
											getFirstChild().getNodeValue();
									if (periodNumber == 1) {
										refStartDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(periodStart));
										refEndDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(periodEnd));
									}
									else {addRefRead(periodStart, periodEnd);}
								}
							}
						}
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:spatialresolution")) {
							if (dataIdentification.item(bb).getFirstChild().getFirstChild().getNodeName().
								...
								try {
									geoResolutionBox.setText(dataIdentification.item(bb).getFirstChild().getFirstChild().getFirstChild().
								...
								try {
									...
									geoResolutionBox.setText(dataIdentification.item(bb).getFirstChild().getFirstChild().getFirstChild().
											getFirstChild().getNodeValue());
									geoUnitBox.setText(((Element)dataIdentification.item(bb).getFirstChild().getFirstChild().getFirstChild()).
											getAttribute("uom"));
								}
								catch (Exception ex) {}
							}
							...
							for (int gg = 0; gg < useConditions.getLength(); gg++) {
								if (useConditions.item(gg).getNodeName().startsWith("gmd:uselimitation")) {
									...
									if (useConditionNumber == 1) {
										try {useConditionsBox.setText(useConditions.item(gg).getFirstChild().getFirstChild().getNodeValue());}
										catch (Exception ex) {}
									}
									else {
										addUseRead(useConditionsAddTab, useConditionsLst, useConditions.item(gg).getFirstChild().getFirstChild().
												getNodeValue());
									}
								}
								if (useConditions.item(gg).getNodeName().startsWith("gmd:otherconstraints")) {
									...
									if (accessLimitationNumber == 1) {
										try {useLimitationsBox.setText(useConditions.item(gg).getFirstChild().getFirstChild().getNodeValue());}
										catch (Exception ex) {}
									}
									else {
										addUseRead(useLimitationsAddTab, useLimitationsLst, useConditions.item(gg).getFirstChild().
												getFirstChild().getNodeValue());
									}
								}
							}
						...
						if (dataIdentification.item(bb).getNodeName().startsWith("gmd:pointofcontact")) {
							...
							if (responsibleParty == 1) {
								try {
									orgPartyBox.setText(responsiblePartyItems.item(0).getFirstChild().getFirstChild().getNodeValue());
									orgEmailBox.setText(responsiblePartyItems.item(1).getFirstChild().getFirstChild().getFirstChild().
											getFirstChild().getFirstChild().getFirstChild().getNodeValue());
									checkList(roleMap, responsiblePartyItems.item(2).getFirstChild().getFirstChild().getNodeValue(), orgRoleLst);
								}
								catch (Exception ex) {}
							} else {
								addOrgRead(responsiblePartyItems.item(0).getFirstChild().getFirstChild().getNodeValue(), responsiblePartyItems.
				...
				if (docChildNodes.item(aa).getNodeName().startsWith("gmd:distributioninfo")) {
					try {
						identLocatorBox.setText(docChildNodes.item(aa).getFirstChild().getFirstChild().getFirstChild().getFirstChild().
				...
				if (docChildNodes.item(aa).getNodeName().startsWith("gmd:language")) {
					checkList(languageMap, docChildNodes.item(aa).getFirstChild().getFirstChild().getNodeValue(), metLanguageLst);
				}
				if (docChildNodes.item(aa).getNodeName().startsWith("gmd:datestamp")) {
					metDateDat.setValue(DateTimeFormat.getFormat("yyyy-MM-dd").parse(docChildNodes.item(aa).getFirstChild().getFirstChild().
							getNodeValue()));
				}
				...
				if (docChildNodes.item(aa).getNodeName().startsWith("gmd:contact")) {
					...
					if (metadataContact == 1) {
						try {
							...
							metEmailBox.setText(contactNodes.item(1).getFirstChild().getFirstChild().getFirstChild().getFirstChild().
									getFirstChild().getFirstChild().getNodeValue());
						}
						catch (Exception ex) {}
					} else {
						NodeList contactNodes = docChildNodes.item(aa).getFirstChild().getChildNodes();
						addMetRead(contactNodes.item(0).getFirstChild().getFirstChild().getNodeValue(),contactNodes.item(1).getFirstChild().
				...
				if (docChildNodes.item(aa).getNodeName().startsWith("gmd:acquisitioninfo")) {
					String registrationNumber = docChildNodes.item(aa).getFirstChild().getFirstChild().getLastChild().getFirstChild().
							getFirstChild().getNodeValue();
					int index = -1;
	...
	private void runCheck(final String string) {
		...
		for (Map.Entry<TextBoxBase, Label> entry : requiredField.entrySet()) {
			...
			while (!(parent instanceof ScrollPanel)) {parent = parent.getParent();}
			if (textBox.getText() == "") {
		...
		for (int i = 0; i < allClaCheck.size(); i++) {
			if  (allClaCheck.get(i).getValue() == true) {allClaCheckNumber++;}
		}
		...
		for (int i = 0; i < allKeyCheck.size(); i++) {if (allKeyCheck.get(i).getValue() == true) {allKeyCheckNumber++;}}
		...
		if (geoResolutionLst.getSelectedIndex() == 1) {if (geoUnitBox.getText() != "") {if (!runCorrect(geoUnitBox)) {notCompleted++;}}}
		List<DateBox> allDateBox = $("*", subDockPanel).widgets(DateBox.class);
		for (int i = 0; i < allDateBox.size(); i++) {if (!runCorrect(allDateBox.get(i))) {notCompleted++;}}
		if (notCompleted > 0) {result = false;}
		if (!result) {
			final DialogBox infoDialog = new DialogBox();
			final VerticalPanel verticalPanel01 = new VerticalPanel();
			final HorizontalPanel horizontalPanel01 = new HorizontalPanel();
			...
			verticalPanel01.add(horizontalPanel02);
			label.getElement().setAttribute("style", "margin-left: 10px !important;");
			buttonOk.getElement().setAttribute("style", "margin-left: 105px !important; font-family: DroidSansFallback !important; font-weight: "
					+ "bold !important; margin-top: 10px !important;");
	...
	private boolean runCorrect(final TextBoxBase textBox) {
		...
		for (Map.Entry<TextBoxBase, String> entry : correctField.entrySet()) {
			...
			if (textBoxE == textBox) {
				switch (string) {
				case "number":
					try { Double.parseDouble(textBox.getText());} 
					catch (NumberFormatException e) {
						textBox.getElement().setAttribute("style","border-color: blue !important;");
						result = false;
						for (Map.Entry<TextBoxBase, Label> entry2 : requiredField.entrySet()) {
							Label label2 = entry2.getValue();
							TextBoxBase textBoxE2 = entry2.getKey();
							parent = label2.getParent();
							while (!(parent instanceof ScrollPanel)) {parent = parent.getParent();}
							if (textBoxE2 == textBox){
								label2.getElement().setAttribute("style","color: rgb(0,0,200) !important;");
								widgetIndex = stackPanel.getWidgetIndex(parent);
								stackPanel.getHeaderWidget(widgetIndex).asWidget().setStylePrimaryName("textuncorrect");
								break;
							}
				...
				case "email":
					...
					for (char ch: textBox.getText().toCharArray()) {if (ch == '@') {isEmail = true;}}
					if (!isEmail) {
						textBox.getElement().setAttribute("style","border-color: blue !important;");
						result = false;
						for (Map.Entry<TextBoxBase, Label> entry2 : requiredField.entrySet()) {
							...
							while (!(parent instanceof ScrollPanel)) {parent = parent.getParent();}
							if (textBoxE2 == textBox){
								label2.getElement().setAttribute("style","color: rgb(0,0,200) "
										+ "!important;");
								widgetIndex = stackPanel.getWidgetIndex(parent);
								stackPanel.getHeaderWidget(widgetIndex).asWidget().setStylePrimaryName(""
										+ "textuncorrect");
								break;
							}
						}
			...
			if (refStartLst.contains(dateBox) || refEndLst.contains(dateBox)) {
				for (int row = 0; row < refPhaseTab.getRowCount(); row++) {
					for (int col = 0; col < refPhaseTab.getCellCount(row); col++) {
						...
						if (w == dateBox) {refPhaseTab.getWidget(row, 0).getElement().setAttribute("style","color: rgb(0,0,200) !important;");}
					}
				}
			} else {
	...
	private void runCheckDefault() {
		List<Label> allLabel = $("*", subDockPanel).widgets(Label.class);
		List<TextBoxBase> allBox = $("*", subDockPanel).widgets(TextBoxBase.class);
		List<DateBox> allDateBox = $("*", subDockPanel).widgets(DateBox.class);
		for (int i = 0; i < allLabel.size(); i++) {allLabel.get(i).getElement().setAttribute("style","color: black !important;");}
		for (int i = 0; i < allBox.size(); i++) {allBox.get(i).getElement().setAttribute("style","border-color: #ccc !important;");}
		for (int i = 0; i < 10; i++) {stackPanel.getHeaderWidget(i).asWidget().setStylePrimaryName("textcompleted");}
		for (int i = 0; i < allDateBox.size(); i++) {allDateBox.get(i).getElement().setAttribute("style","border-color: #ccc !important;");}
	...
	private void docNotModified() {
		isModified = false;
		if (titleString.contains("modified")) {titleString = titleString.substring(0, 22);}
		Window.setTitle(titleString);
	}
}
